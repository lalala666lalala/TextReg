...
from .optimizer_prompts import construct_tgd_prompt, OPTIMIZER_SYSTEM_PROMPT, GRADIENT_TEMPLATE, \
    ...
    def step(self):
        ...
        for parameter in self.parameters:
            ...
            # Check if we got a cannot be indexed error
                ...
                raise IndexError(
                    f"TextualGradientDescent optimizer response could not be indexed. This can happen if the optimizer model cannot follow the instructions. You can try using a stronger model, or somehow reducing the context of the optimization. Response: {new_text}")
...
class TextualGradientDescentwithMomentum(Optimizer):
    ...
    def _update_momentum_storage(self, variable: Variable, momentum_storage_idx: int):
        ...
        self.momentum_storage[momentum_storage_idx].append(
    ...
    def step(self):
        for idx, parameter in enumerate(self.parameters):
            ...
            new_text = self.engine(prompt_update_parameter, system_prompt=self.optimizer_system_prompt)
            logger.info(f"TextualGradientDescentwithMomentum optimizer response",
                        extra={"optimizer.response": new_text})
            try:
                new_value = new_text.split(self.new_variable_tags[0])[1].split(self.new_variable_tags[1])[0].strip()
            # Check if we got a cannot be indexed error
