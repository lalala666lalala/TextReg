...
def parse_multiple_choice_answer(text: str) -> Optional[str]:
    """
    Robust helper to extract the last multiple-choice option letter from text.
    Prefers (X) format; falls back to standalone letter tokens.
    """
    try:
        last_line = text.strip().upper().split('\n')[-1]

        # Primary: look for the most reliable `(X)` format
        matches_with_parens = re.findall(r'\(([A-L])\)', last_line)
        if matches_with_parens:
            return matches_with_parens[-1]

        # Fallback: scan backwards for standalone letter tokens
...
def multiple_choice_equality_fn(prediction: tg.Variable, ground_truth_answer: tg.Variable):
    """
    Evaluation function for multiple-choice answers.
    Extracts and compares the option letter from both prediction and ground truth.
    """
...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    elif ("tracking_shuffled_objects" in task_name) or ("logical_deduction" in task_name) or ("temporal_sequences" in task_name) or ("reasoning_about_colored_objects" in task_name):
        from textgrad.loss import MultiFieldTokenParsedEvaluation
        from .big_bench_hard import BigBenchHard, string_based_equality_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction
